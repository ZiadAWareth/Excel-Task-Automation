...
def Execute(path_in):
    ...
    # getting pgw numbers
    def get_pgw_value(index): 
        return sheet_payment_df['totalAmount'][index]-sheet_payment_df['walletAmount'][index]
    def get_amazon_commission(index):
        return sheet_payment_df['PGW'][index] * 0.0095 +1.5
    def get_gedia_commission(index):
        return sheet_payment_df['PGW'][index]* 0.01+ 1.5    
    def get_fawry_commission(index):
        x= sheet_payment_df['PGW'][index]*0.0115  *1.14  
        if(x>28.5):
            return 28.5
        else: return x
...
# puttin pgw numbers
    for index, row in sheet_payment_df.iterrows():
        pgw = get_pgw_value(index)
        provider = row['provider']
        
        sheet_payment_df['Payment_Gateway'][index]= pgw
        if(row['provider'] == 'AMAZONPAY'):
            comm = get_amazon_commission(index)
        if(row['provider']=='GEIDEABANKMASR'):
            comm = get_gedia_commission(index)
        if(row['provider'] == 'PAYATFAWRY'):
            comm = get_fawry_commission(index)
        sheet_payment_df['Comission'][index]=comm
    sheet1_2_df = pandas.read_excel(sheet1_file,sheet_name='Sheet1')
    sheet1_2_df['Unnamed: 6'][4]=sheet1_fawry_df['Net Amount'].sum(skipna=True)
    sheet1_2_df['Unnamed: 7'][4]=sheet1_fawry_df['Count'].sum(skipna=True)
    sheet1_2_df['Unnamed: 8'][4]=sheet1_fawry_df['Merchant Commission'].sum(skipna=True)
...
# getting gedia numbers
    ...
    print('x')
    sheet1_2_df['Unnamed: 6'][3]=actual_geida_amount_sum
    sheet1_2_df['Unnamed: 7'][3]=actual_geida_count
    sheet1_2_df['Unnamed: 8'][3]=actual_geida_com
    ...
    for index, row in sheet1_amazon_df.iterrows():
        sheet1_amazon_df['Amount'][index]= float(sheet1_amazon_df['Amount'][index])
        sheet1_amazon_df['Comission'][index]= get_amazon_num(float(row['Amount']))
    sheet1_2_df['Unnamed: 6'][2]=sheet1_amazon_df['Amount'].sum()
    sheet1_2_df['Unnamed: 7'][2]=sheet1_amazon_df['Count'].sum()
    sheet1_2_df['Unnamed: 8'][2]=sheet1_amazon_df['Comission'].sum()
    sheet1_2_df['Unnamed: 6'][6] = sheet1_2_df['Unnamed: 6'][2]+sheet1_2_df['Unnamed: 6'][3]+sheet1_2_df['Unnamed: 6'][4]
    sheet1_2_df['Unnamed: 7'][6] = sheet1_2_df['Unnamed: 7'][2]+sheet1_2_df['Unnamed: 7'][3]+sheet1_2_df['Unnamed: 7'][4]
    sheet1_2_df['Unnamed: 8'][6] = sheet1_2_df['Unnamed: 8'][2]+sheet1_2_df['Unnamed: 8'][3]+sheet1_2_df['Unnamed: 8'][4]
    def get_sums():
        amazon_sum=0
        ...
        for index , row in sheet_payment_df.iterrows():
            if(row['provider'] == 'AMAZONPAY'):
                ...
                amazon_sum = amazon_sum+row['PGW']
                amazon_commission = amazon_commission+ row['C']
            if(row['provider'] == 'GEIDEABANKMASR'):
                gedia_order_count=gedia_order_count+1
                gedia_sum = gedia_sum+row['PGW']
                gedia_comission= gedia_comission+row['C']
            if(row['provider'] == 'PAYATFAWRY'):
                fawry_order_count=fawry_order_count+1
                fawry_sum_sum = fawry_sum_sum+row['PGW']
                fawry_comission = fawry_comission + row['C']
    ...
    sheet1_2_df['Unnamed: 1'][2] = list_of_sums[0]
    sheet1_2_df['Unnamed: 2'][2] = list_of_sums[1]
    sheet1_2_df['Unnamed: 3'][2] = list_of_sums[2]
    sheet1_2_df['Unnamed: 1'][3] = list_of_sums[3]
    sheet1_2_df['Unnamed: 2'][3] = list_of_sums[4]
    sheet1_2_df['Unnamed: 3'][3] = list_of_sums[5]
    sheet1_2_df['Unnamed: 1'][4] = list_of_sums[6]
    sheet1_2_df['Unnamed: 2'][4] = list_of_sums[7]
    sheet1_2_df['Unnamed: 3'][4] = list_of_sums[8]

    sheet1_2_df['Unnamed: 1'][6] = list_of_sums[0]+list_of_sums[3]+list_of_sums[6]
    sheet1_2_df['Unnamed: 2'][6] = list_of_sums[1]+list_of_sums[4]+list_of_sums[7]
    sheet1_2_df['Unnamed: 3'][6] = list_of_sums[2]+list_of_sums[5]+list_of_sums[8]
